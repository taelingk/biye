...
import sys, tempfile, shutil, logging
...
def generate_mock_subject(fs_ecg=500, fs_ppg=100, fs_scg=800, duration_sec=60, subject_id="mock_001"):
    ...
    clinical = {
        "age": 30, "gender": 1, "weight_kg": 70, "height_cm": 175,
        "hr_bpm": 70, "sbp": 120, "dbp": 80,
    }
    co_true = np.array([5.0])
    vo2_true = np.array([35.0])

    return {"ecg": ecg, "ppg": ppg, "scg": scg, "clinical": clinical,
            "co_true": co_true, "vo2_true": vo2_true,
            "fs_ecg": fs_ecg, "fs_ppg": fs_ppg, "fs_scg": fs_scg}

# --- 2. Run preprocessing ---
from src.cardiofit.preprocessing import process_ecg, process_ppg, process_scg, extract_windows, compute_clinical_features

logger.info("=== Phase 1: Preprocessing ===")
mock = generate_mock_subject()
logger.info(f"Mock subject: ECG={len(mock['ecg'])}, PPG={len(mock['ppg'])}, SCG={len(mock['scg'])}")
...
logger.info(f"Resampled: ECG={len(ecg_125)}, PPG={len(ppg_125)}, SCG={len(scg_125)}, R-peaks={len(r_peaks)}")

if len(r_peaks) == 0:
    r_peaks = np.array([50, 175, 300, 425, 550], dtype=int)  # fallback

windows = extract_windows(ecg_125, ppg_125, scg_125, r_peaks, window_size=125, before_r=40)
...
from src.cardiofit.dataset import load_subject, build_tf_dataset, split_subjects, fit_standardization_params
...
logger.info(f"Loaded: signals={signals.shape}, clinical={clinical.shape}, co={co.shape}, vo2={vo2.shape}")
...
ds = build_tf_dataset(splits["train"], proc_dir, batch_size=2, shuffle=False, augment=False)
for batch in ds.take(1):
    sig, clin = batch[0]
    co_b, vo2_b = batch[1]
    logger.info(f"Batch: signal={sig.shape}, clinical={clin.shape}, co={co_b.shape}, vo2={vo2_b.shape}")
...
print(f"  - HDF5 I/O: write → read → tf.data pipeline")
